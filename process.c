...
void add_process_record(int pid, int arrival_time, int burst_time, int priority, int io_count, int* request_times, int* burst_times){
    ...
    if (io_count > 0 && io_count < MAX_IO + 1){
        fill_io_entries(p->io, p->io_count, request_times, burst_times);
    }
    p->current_io = 0;
    p->cpu_time = 0;
}
...
void add_random_process_record(int pid){
    ...
    fill_random_io_entries(p->io, p->io_count, p->burst_time);
    p->current_io = 0;
    p->cpu_time = 0;
}
...
void load_process_records(int choice){
    ...
    if (choice == 1){
        add_process_record(0, 0, 10, 2, 0, NULL, NULL);
        add_process_record(0, 2, 5, 3, 0, NULL, NULL);
        add_process_record(0, 4, 3, 1, 0, NULL, NULL);
    } else if (choice == 2){
        for (int i = 0; i < num; i++){
            add_random_process_record(0);
        }
    } else if (choice == 3){
        for (int i = 0; i < num; i++){
            char path[32];
            ...
            if (io_count > 0){
                ...
                fclose(fp);
                add_process_record(pid, arrival, burst, priority, io_count, io_request, io_burst);
                if (io_count > 0){
                    free(io_request);
                    free(io_burst);
                    io_request = NULL;
